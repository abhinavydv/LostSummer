...
class Ground(Collection):
    ...
    def get_added_to_screen(self, levelscreen):
        ...
        num_centers = int((self.relative_width - left.relative_width) /
                          (self.relative_height * width / height))
        ...
        for i in range(num_centers):
            ...
            center.relative_position[0] = self.relative_position[0] + \
                left.relative_width + i * center.relative_width
...
class Pipe(Collection):
    ...
    def get_added_to_screen(self, levelscreen):
        if self.has_piranha:
            ...
            piranha.relative_position[0] += (self.relative_width -
                                             piranha.relative_width)/2
            piranha.relative_position[1] += (self.relative_height -
                                             piranha.relative_height)
            piranha.relative_velocity = list(
                levelscreen.sprites["Piranha"]["relative_velocity"])
            piranha.max_relative_velocity = list(
                levelscreen.sprites["Piranha"]["max_relative_velocity"])
...
class QuestionBlock(StaticSprite):
    ...
    def on_collide(self, other: object, col):
        super().on_collide(other, col)
        if not self.is_dead:
            if col[2] == "bottom" and other.tag == "player":
                self.is_dead = True
                self.activate()
    ...
    def activate(self):
        ...
        if self.content == "Mushroom":
            if not levelscreen.player.small:
        ...
        content["relative_position"][0] += (
            self.relative_width-content["relative_width"])/2
        content["relative_position"][1] += (
            self.relative_height-content["relative_height"])/2
        content["relative_screen_position"] = self.relative_screen_position

        import importlib
        Collectible = getattr(importlib.import_module(
            "widgets.collectibles"), self.content)
...
class GroundDarkUpLeft(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileUpLeft.png"


class GroundDarkUpRight(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileUpRight.png"


class GroundDarkUpCenter(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileUpCenter.png"


class GroundDarkMidLeft(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileMidLeft.png"


class GroundDarkMidRight(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileMidRight.png"


class GroundDarkMidCenter(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileMidCenter.png"


class GroundDarkDownLeft(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileDownLeft.png"


class GroundDarkDownRight(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileDownRight.png"


class GroundDarkDownCenter(StaticSprite):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/TileDownCenter.png"


class GroundDarkFloating(Collection):

    def get_added_to_screen(self, levelscreen):

        # GroundLeft
        left = GroundDarkDownLeft()
        self.sprites["GroundLeft"] = left
        left.relative_height = self.relative_height
        left.relative_position = self.relative_position
        width, height = levelscreen.images[left.image]
...
class GroundDark(Collection):
    
    def get_added_to_screen(self, levelscreen):

        # GroundLeft
        left = GroundDarkUpLeft()
        self.sprites["GroundLeft"] = left
        left.relative_height = self.relative_height
        left.relative_position = self.relative_position
        width, height = levelscreen.images[left.image]
        ...
        # GroundCenter
        width, height = levelscreen.images["assets/images/Tiles/TileUpCenter.png"]
        num_centers = int((self.relative_width - left.relative_width) /
                          (self.relative_height * width / height))

        for i in range(num_centers):
            center = GroundDarkUpCenter()
            self.sprites[f"GroundCenter{i}"] = center
            center.relative_height = self.relative_height
            center.relative_width = center.relative_height * width / height
            center.relative_position[1] = self.relative_position[1]
...
class Background(StaticSprite):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.image = "assets/images/Tiles/BG.png"
        self.collider = False
